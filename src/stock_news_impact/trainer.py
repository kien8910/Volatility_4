...
class Step7RunConfig:
    ...
    def config_id(self) -> str:
        ...
        return f"{self.model}__p{self.initial_probability:g}__rh{hidden}__uw{self.usage_weight:g}__cw{self.correction_weight:g}"
...
def predict_event_rows(model, frame: pd.DataFrame, columns: dict[str, list[str]], cfg: dict, device: torch.device, model_name: str) -> pd.DataFrame:
...
def train_gate_model(run_cfg: Step7RunConfig, train_frame: pd.DataFrame, val_frame: pd.DataFrame, columns: dict[str, list[str]], cfg: dict, device: torch.device, ckpt_dir: str | Path):
    ...
    if run_cfg.model == "S2_FixedSmallGate":
        return predict_event_rows(model, val_frame, columns, cfg, device, run_cfg.model), {"best_epoch": 0, "best_metric": np.nan}
    ...
    for epoch in range(int(cfg["training"]["max_epochs"])):
        ...
        for batch in loader:
            ...
            optimizer.step()
        pred = prediction_level_from_event_rows(predict_event_rows(model, val_frame, columns, cfg, device, run_cfg.model), cfg)
        ...
        if metric < best_metric:
            ...
            if stale >= patience:
                break
    return predict_event_rows(model, val_frame, columns, cfg, device, run_cfg.model), {"best_epoch": best_epoch, "best_metric": best_metric}
